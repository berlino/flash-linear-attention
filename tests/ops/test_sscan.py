...
def test_sscan(dtype, d_v, L, d_k=16, bs=4):
    ...
    # initial_hidden_state = torch.randn((bs, d_v, d_k), dtype=dtype, device='cuda').requires_grad_(True)
    initial_hidden_state = None

    do = torch.randn_like(v) 
    ref, next_hidden_state = naive_selective_scan(v, delta, A, B, C, D, initial_hidden_state)
    ...
    # ref_initial_hidden_state, initial_hidden_state.grad = initial_hidden_state.grad.clone(), None

    tri, next_hidden_state = selective_scan_block_parallel(v, delta, A, B, C, D, initial_hidden_state)
    ...
    # tri_initial_hidden_state, initial_hidden_state.grad = initial_hidden_state.grad.clone(), None

    breakpoint()
    assert ref.allclose(tri, 0, 1e1)
    assert ref_dv.allclose(tri_dv, 0, 1e1)
    assert ref_dA_prime.allclose(tri_dA_prime, 0, 1e1)
    assert ref_dB.allclose(tri_dB, 0, 1e1)
    assert ref_dC.allclose(tri_dC, 0, 1e1)
    assert ref_dD.allclose(tri_dD, 0, 1e1)
    assert ref_ddelta_prime.allclose(tri_ddelta_prime, 0, 1e-1)
    # assert ref_initial_hidden_state.allclose(tri_initial_hidden_state, 0, 1e-1)
