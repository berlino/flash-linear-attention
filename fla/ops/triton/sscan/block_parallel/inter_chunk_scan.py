...
def _bwd_recurrence(
    ...
    X = X + offset_bn * NUM_BLOCK * D_MODEL_K * D_MODEL_V + tl.arange(0, D_MODEL_K)[:, None] * D_MODEL_V + offset_v * BLOCK_MODEL + tl.arange(0, BLOCK_MODEL)[None, :]  + (NUM_BLOCK - 2) * D_MODEL_K * D_MODEL_V
    ...
    for _ in range(NUM_BLOCK - 1):
        ...
        # Dacc means F_t \odot \nabla_{C_t+1} L in the note 
        dF_i = Dacc * X_i
        tl.store(dF, dF_i.to(X.dtype.element_ty))
        tl.store(X, Dacc.to(X.dtype.element_ty))        
...
class InterChunkScan(torch.autograd.Function):
    ...
    def forward(ctx, X, F, initial_state):
        ...
        ctx.save_for_backward(H, F, initial_state)        
...
def naive_inter_chunk_scan(X, F, initial_state=None):
    ...
    H = torch.zeros(bs, num_chunk, d_k, d_v, dtype=X.dtype, device=X.device)
    if initial_state is None:
    ...
    for i in range(num_chunk + 1):
        if i == 0:
            ...
            final_state = F[:, i-1] * H[:, i-1] + X[:, i-1]
        else:
            H[:, i] = F[:, i-1] * H[:, i-1] + X[:, i-1]
...
if __name__ == "__main__":
    ...
    tri, tri_final_state = InterChunkScan.apply(X2, F, initial_state)
    tri.backward(do)
    tri_dX, X2.grad = X2.grad.clone(), None
    tri_dF, F.grad = F.grad.clone(), None

    assert ref.allclose(tri, 0, 1e1)
    assert ref_final_state.allclose(tri_final_state, 0, 1e1)
    assert ref_dX.allclose(tri_dX, 0, 1e1)
    assert ref_dF.allclose(tri_dF, 0, 1e1)
